...
class MPCIncremental:
    def __init__(self, m, q, r, A, B, C, D0, F, Psi, N, nx, nu, ny, nd, ymax, ymin, dumax, umax, umin, xmk, uk_1, utg, qu, sy, su):
        self.m = m
        self.q = q
        self.r = r
        self.A = A
    ...
    def control(self):
        ...
        utg   = np.array(self.utg, ndmin=2, dtype=float).transpose()


        q = np.array(self.q, ndmin=2, dtype=float)
        r = np.array(self.r, ndmin=2, dtype=float)
        qu = np.array(self.qu, ndmin=2, dtype=float)
        su = np.array(self.su, ndmin=2, dtype=float)
        sy = np.array(self.sy, ndmin=2, dtype=float)

        # Phi: predição livre (matriz de observabilidade estendida)
        Phi = np.vstack([np.dot(self.C, np.linalg.matrix_power(self.A, k)) for k in range(1, self.m+1)])
        # Theta: resposta ao impulso (matriz de controle preditivo)
        tha = np.vstack([multi_dot([self.C, matrix_power(self.A, k-1), self.B]) for k in range(1, self.m+1)])
        Co = np.hstack([np.linalg.matrix_power(self.A, self.m-j)@self.B for j in range(1, self.m+1)])
        
        aux1 = np.tile(q, (self.m, 1))
        aux2 = np.tile(r, (self.m, 1))
        axu3 = np.tile(qu, (self.m, 1))

        a = tha.copy()
        Theta = a.copy()
        for _ in range(self.m-1):
            a = np.vstack([np.zeros((self.ny, self.nu)), a[0:(self.m-1)*self.ny, :]])
            Theta = np.hstack([Theta, a])

        Qybar = np.diag(aux1.flatten())
        Rbar = np.diag(aux2.flatten())
        Qubar = np.diag(axu3.flatten())
        Sy = np.diag(sy.flatten())
        Su = np.diag(su.flatten())
        Qy = np.diag(q.flatten())
        Qaux = self.F.T @ self.Psi.T @ Qy @ self.Psi @ self.F
        Qbar = solve_discrete_lyapunov(self.F, Qaux)
        ...
        Ital = np.tile(np.eye(self.ny), (self.m, 1))
        Ibar = np.vstack([np.eye(self.nu), np.zeros((self.nu * (self.m - 1), self.nu))])
        Ns = np.hstack((np.eye(self.ny), np.zeros((self.ny, self.nd))))  # dimensão (ny x nx)
        Nd = np.hstack((np.zeros((self.nd, self.ny)), np.eye(self.nd)))  # dimensão (nd x nx)

        # Matriz Hessiana
        H11 = Theta.T@Qybar@Theta + Co.T@Nd.T@Qbar@Nd@Co + Mtil.T@Qubar@Mtil + Rbar
        H12 = -Theta.T @ Qybar @ Ital
        H13 = H12
        H14 = -Mtil.T @ Qubar @ Itil

        H21 = H12.T
        H22 = Ital.T @ Qybar @ Ital
        H23 = Ital.T @ Qybar @ Ital
        H24 = np.zeros((self.ny, self.nu))

        H31 = H13.T
        H32 = H23.T
        H33 = Ital.T @ Qybar @ Ital + Sy
        H34 = np.zeros((self.ny, self.nu))

        H41 = H14.T
        H42 = H24.T
        H43 = H34.T
        H44 = Itil.T @ Qubar @ Itil + Su


        H = np.vstack((np.hstack((H11, H12, H13, H14)),
                       np.hstack((H21, H22, H23, H24)),
                       np.hstack((H31, H32, H33, H34)),
                       np.hstack((H41, H42, H43, H44))
                      ))
 
        H = (H + H.T)/2
        
        cf1 = (self.xmk.T @ Phi.T @ Qybar @ Theta + self.xmk.T @ np.linalg.matrix_power(self.A, self.m).T @ Nd.T @ Qbar @ Nd @ Co + (self.uk_1 - utg).T @ Itil.T @ Qubar @ Mtil)
        cf2 = -self.xmk.T @ Phi.T @ Qybar @ Ital
        cf3 = cf2
        cf4 = (utg - self.uk_1).T @ Itil.T @ Qubar @ Itil

        cf = np.hstack([cf1, cf2, cf3, cf4]).T

        c = self.xmk.T@Phi.T@Qybar@Phi@self.xmk + self.uk_1.T@Ital.T@Rbar@Ital@self.uk_1
        c = (self.xmk.T@ (Phi.T @ Qybar @ Phi + np.linalg.matrix_power(self.A, self.m).T @ 
                          Nd.T @ Qbar @ Nd @ np.linalg.matrix_power(self.A, self.m))
                          @ self.xmk + (self.utg - self.uk_1).T @ Itil.T @ Qubar @ Itil @ (self.utg - self.uk_1))

        # Restrições de desigualdade Ain*x <= Bin
        Ain = np.vstack((np.hstack((Mtil, np.zeros((self.m * self.nu, self.ny)), np.zeros((self.m * self.nu, self.ny)), np.zeros((self.m * self.nu, self.nu)))),
                         np.hstack((-Mtil, np.zeros((self.m * self.nu, self.ny)), np.zeros((self.m * self.nu, self.ny)), np.zeros((self.m * self.nu, self.nu)))),
                         np.hstack((np.eye((self.m * self.nu)), np.zeros((self.m * self.nu, self.ny)), np.zeros((self.m * self.nu, self.ny)), np.zeros((self.m * self.nu, self.nu)))),
                         np.hstack((-np.eye((self.m * self.nu)), np.zeros((self.m * self.nu, self.ny)), np.zeros((self.m * self.nu, self.ny)), np.zeros((self.m * self.nu, self.nu)))),
                         np.hstack((np.zeros((self.ny, self.m * self.nu)), np.eye((self.ny)), np.zeros((self.ny, self.ny)), np.zeros((self.ny, self.nu)))),
                         np.hstack((np.zeros((self.ny, self.m * self.nu)), -np.eye((self.ny)), np.zeros((self.ny, self.ny)), np.zeros((self.ny, self.nu)))))
                        )
        ...
        Aeq = np.vstack((np.hstack((Ns@Co, -np.eye(self.ny), -np.eye(self.ny), np.zeros((self.ny, self.nu)))),
                         np.hstack((Itil.T, np.zeros((self.nu, self.ny)), np.zeros((self.nu, self.ny)), -np.eye(self.nu)))
                         ))
        Beq = np.vstack([-Ns@(np.linalg.matrix_power(self.A, self.m)@self.xmk),
                         utg-self.uk_1])
            
        qp_opt = {'show_progress': False}
        solution = solvers.qp(matrix(H), matrix(cf), matrix(Ain), matrix(Bin), matrix(Aeq), matrix(Beq), options=qp_opt)
        x = np.array(solution['x'])
        duk = x[0:self.nu]
        ysp = x[self.m*self.nu:self.m*self.nu+self.ny]
        sky = x[self.m*self.nu+self.ny:self.m*self.nu+2*self.ny]
        sku = x[self.m*self.nu+2*self.ny:self.m*self.nu+2*self.ny+self.nu]
        vk = float((x.T @ H @ x + 2 * cf.T @ x + c)[0, 0])
        ...
        return duk, ysp, sky, sku, vk, status_code
