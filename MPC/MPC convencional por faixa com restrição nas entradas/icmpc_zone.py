...
class MPCIncremental:
    def __init__(self, m, p, q, r, A, B, C, nx, nu, ny, ymax, ymin, dumax, umax, umin, xmk, uk_1):
        self.m = m
        self.p = p
        self.q = q
        self.r = r
    ...
    def control(self):
        ...
        Ital = np.tile(np.eye(self.ny), (self.p, 1))
        Ibar = np.vstack([np.eye(self.nu), np.zeros((self.nu * (self.m - 1), self.nu))])

         # Matriz Hessiana
        H11 = Theta.T@Qbar@Theta + Rbar #uk,uk
        H12 = -Theta.T@Qbar@Ital #ukysp

        H21 = H12.T #ysp,uk
        H22 = Ital.T@Qbar@Ital #ysp,ysp
        ...
        cf1 = self.xmk.T @ Phi.T @ Qbar @ Theta
        cf2 = -self.xmk.T@Phi.T@Qbar@Ital #ysp

        cf = np.hstack([cf1, cf2]).T

        c = self.xmk.T@Phi.T@Qbar@Phi@self.xmk 

        # Restrições de desigualdade Ain*x <= Bin
        Ain = np.vstack((np.hstack((Mtil, np.zeros((self.m * self.nu, self.ny)))),
                         np.hstack((-Mtil, np.zeros((self.m * self.nu,self.ny )))),
                         np.hstack((np.eye((self.m * self.nu)), np.zeros((self.m * self.nu, self.ny)))),
                         np.hstack((-np.eye((self.m * self.nu)), np.zeros((self.m * self.nu, self.ny)))),
                         np.hstack((np.zeros((self.ny, self.m * self.nu)), np.eye((self.ny)))),
                         np.hstack((np.zeros((self.ny, self.m * self.nu)), -np.eye((self.ny))))
                        ))
...
class Plot:
    ...
    def plotar(self, x, y_list, labels, xlabel, ylabel, titulo=None, cores=None, marcadores=None, estilos=None,
        ...
        if subplots:
            ...
            axs[0].xaxis.set_major_locator(plt.MultipleLocator(max(1, len(x)//10)))
